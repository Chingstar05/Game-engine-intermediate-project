...
public class Enemy1 : MonoBehaviour
{
    ...
    public GameObject doorPrefab;   // 생성할 Door 프리팹
    private bool doorSpawned = false; // 중복 방지용
    ...
    void ShootProjectile()
    {
        if (projectilePrefab != null && firePoint != null && player != null)
        {
            ...
            // 데미지 정보 전달
            EnemyProjectile1 ep = proj.GetComponent<EnemyProjectile1>();
            if (ep != null)
            {
                ep.damage = damage; // 여기서 데미지 설정
            }

            Destroy(proj, 1f); // 안전하게 파괴
        }
    }
